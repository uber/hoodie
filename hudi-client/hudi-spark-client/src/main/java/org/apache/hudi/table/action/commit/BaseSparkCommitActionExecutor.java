...
public abstract class BaseSparkCommitActionExecutor<T extends HoodieRecordPayload> extends
  ...
  protected boolean populateMetaCols;
  ...
  private void initKeyGenIfNeeded() {
    this.populateMetaCols = config.populateMetaColumns();
    if (!config.populateMetaColumns()) {
      try {
        keyGenerator = (BaseKeyGenerator) HoodieSparkKeyGeneratorFactory.createKeyGenerator(new TypedProperties(config.getProps()));
      } catch (IOException e) {
  ...
  protected HoodieMergeHandle getUpdateHandle(String partitionPath, String fileId, Iterator<HoodieRecord<T>> recordItr) {
    if (table.requireSortedRecords()) {
      ...
      return new HoodieConcatHandle<>(config, instantTime, table, recordItr, partitionPath, fileId, taskContextSupplier, keyGenerator);
    } else {
      return new HoodieMergeHandle<>(config, instantTime, table, recordItr, partitionPath, fileId, taskContextSupplier, keyGenerator);
    }
  }
